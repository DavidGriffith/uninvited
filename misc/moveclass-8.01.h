 ...
 print "]^";
  ...
  if (movement_type~=AIMED_MOVE)
     rfalse;
                            ...
                            ! Can only calculate paths from NPCRoom class to NPCRoom
                            ! class objects, so...
  if (~~(parent(npc) ofclass NPCRoom) || parent(npc)==targetroom)
      rfalse;
  objectloop (i ofclass NPCRoom)
  {  i.number=0;         
     give i ~en_route;      ! Reset all 'NPCRooms'
  }
